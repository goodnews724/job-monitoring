...
```python
# ThreadPoolExecutor = 새 스레드 생성해서 작업 실행
with ThreadPoolExecutor(max_workers=1) as executor:
    future = executor.submit(do_work, context)  # 다른 스레드에서 실행
```

### Greenlet

- **Greenlet**: 경량 스레드 (코루틴)
- 실제 OS 스레드가 아니라, 프로그램 내에서 협력적으로 전환하는 방식
...
```
Browser (브라우저)
├── Context1 (독립 세션) - 쿠키A, 설정A
│   ├── Page1 (탭)
│   └── Page2 (탭)
│
└── Context2 (독립 세션) - 쿠키B, 설정B
    └── Page1 (탭)
```

- **Browser**: 크롬 프로그램 자체
- **Context**: 독립된 브라우저 세션 (쿠키/설정 분리)
- **Page**: 탭 하나
...
[메인 스레드] context 생성
     ↓
[ThreadPoolExecutor] 새 스레드 생성
     ↓
새 스레드에서 context 사용 시도
     ↓
💥 "Cannot switch to a different thread"
```

Context는 생성된 스레드에 묶여있어서, 다른 스레드에서 사용 불가.
